...
<body>
  <div class="container">
    ...
    $sql_select = $wpdb->get_results($wpdb->prepare("
     SELECT Оквэд3Уровень.КодОквэд3Уровень, КодыОквэд.НазваниеОквэд 
     FROM Оквэд3Уровень
     INNER JOIN КодыОквэд
     ON Оквэд3Уровень.КодОквэд3Уровень = КодыОквэд.КодОквэд
     WHERE Оквэд3Уровень.КодОквэд2Уровень = %s
     ORDER BY Оквэд3Уровень.КодОквэд3Уровень", $code));
    ...
    if ($sql_select) {
      ...
      foreach ($sql_select as $row) {
        ...
        echo '<tr>
        <td style="color: rgb(0, 123, 255);">' . $row->КодОквэд3Уровень . '</td>
        <td style="color: rgb(0, 123, 255);">' . $row->НазваниеОквэд . '</td>
        </tr>';
