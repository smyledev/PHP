...
<body>
  ...
  // Getting params from user entering in form...
    $code_of_competency_2_level = $_GET['code_of_competency_2_level'];
    
    
  // Sql injection protection...
    sanitize_text_field($code_of_competency_2_level);

    $sql_select = $wpdb->get_results($wpdb->prepare("
      SELECT Компетенции3Уровень.КодКомпетенции3Уровень, КодыКомпетенций.НазваниеКомпетенции
      FROM Компетенции3Уровень
      INNER JOIN КодыКомпетенций
      ON Компетенции3Уровень.КодКомпетенции3Уровень = КодыКомпетенций.КодКомпетенции
      WHERE Компетенции3Уровень.КодКомпетенции2Уровень = %s", [$code_of_competency_2_level]));
    ...
    if ($sql_select) {
      ...
      foreach ($sql_select as $row) {
        echo '<tr> 
        <td style="color: rgb(0, 123, 255);">' . $row->КодКомпетенции3Уровень . '</td>
