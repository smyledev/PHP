...
<body>
  ...
  // Sql injection protection...
    ...
    $sql_select = $wpdb->get_results($wpdb->prepare("
      SELECT Компетенции1Уровень.КодКомпетенции1Уровень, КодыКомпетенций.НазваниеКомпетенции
      FROM Компетенции1Уровень
      INNER JOIN КодыКомпетенций
      ON Компетенции1Уровень.КодКомпетенции1Уровень = КодыКомпетенций.КодКомпетенции
      WHERE Компетенции1Уровень.НазваниеПриоритета = %s", [$priority]));
    ...
    if ($sql_select) {
      ...
      foreach ($sql_select as $row) {
        echo '<tr> 
        <td> <a href="/codes_of_competence_2_level.php?code_of_competency_1_level=' . $row->КодКомпетенции1Уровень . '" target="_blank">' . $row->КодКомпетенции1Уровень . '</a></td>
        <td> <a href="/codes_of_competence_2_level.php?code_of_competency_1_level=' . $row->КодКомпетенции1Уровень . '&competency=&priority=" target="_blank">' . $row->НазваниеКомпетенции . '</a></td></tr>';
      }
